...
public class HarmonizeFormDelegate {
	...
	public HarmonizationData getConvertedData(List<FormDTO> forms) {
		...
		for (FormDTO formDTO : forms) {
			formDTO.getForm().setEncounterType(formDTO.getEncounterType());
			HarmonizationItem item = new HarmonizationItem(formDTO.getForm().getUuid(), formDTO);
			item.setEncountersCount(this.harmonizationFormService.getNumberOfAffectedEncounters(formDTO.getForm()));
			item.setFormFieldsCount(this.harmonizationFormService.getNumberOfAffectedFormFields(formDTO.getForm()));
	...
	public void setHarmonizationStage(HttpSession session, HarmonizationData newMDSForms,
			List<FormDTO> productionItemsToDelete, HarmonizationData productionItemsToExportForm,
			HarmonizationData differentIDsAndEqualUUID, HarmonizationData differentNameAndSameUUIDAndID,
			List<Form> notSwappableForms, List<Form> swappableForms) {
		...
		boolean isFirstStepFormHarmonizationCompleted = newMDSForms.getItems().isEmpty()
				&& productionItemsToDelete.isEmpty();

		this.setIDsAndUUIDsFormHarmonizationStatus(productionItemsToExportForm, differentIDsAndEqualUUID,
				isFirstStepFormHarmonizationCompleted);
		this.setNAMEsDifferencesFormHarmonizationStatus(productionItemsToExportForm, differentNameAndSameUUIDAndID,
				isFirstStepFormHarmonizationCompleted);

		boolean hasSecondStepFormHarmonization = isFirstStepFormHarmonizationCompleted
		...
		session.setAttribute("isFirstStepFormHarmonizationCompleted", isFirstStepFormHarmonizationCompleted);
		session.setAttribute("isUUIDsAndIDsFormHarmonized",
				HarmonizeFormController.IS_IDS_AND_UUID_DIFFERENCES_HARMONIZED);
		session.setAttribute("isNamesFormHarmonized", HarmonizeFormController.IS_NAMES_DIFFERENCES_HARMONIZED);
		session.setAttribute("hasSecondStepFormHarmonization", hasSecondStepFormHarmonization);
	}

	private void setIDsAndUUIDsFormHarmonizationStatus(HarmonizationData productionItemsToExport,
			HarmonizationData differentIDsAndEqualUUID, boolean isFirstStepFormHarmonizationCompleted) {

		HarmonizeFormController.IS_IDS_AND_UUID_DIFFERENCES_HARMONIZED = HarmonizeFormController.IS_IDS_AND_UUID_DIFFERENCES_HARMONIZED
		...
		if (HarmonizeFormController.IS_IDS_AND_UUID_DIFFERENCES_HARMONIZED
				&& productionItemsToExport.getItems().isEmpty() && !differentIDsAndEqualUUID.getItems().isEmpty()) {
			HarmonizeFormController.IS_IDS_AND_UUID_DIFFERENCES_HARMONIZED = false;
		}
	}

	private void setNAMEsDifferencesFormHarmonizationStatus(HarmonizationData productionItemsToExportForm,
			HarmonizationData differentNameAndSameUUIDAndIDForm, boolean isFirstStepFormHarmonizationCompleted) {

		HarmonizeFormController.IS_NAMES_DIFFERENCES_HARMONIZED = HarmonizeFormController.IS_NAMES_DIFFERENCES_HARMONIZED
		...
		if (HarmonizeFormController.IS_NAMES_DIFFERENCES_HARMONIZED && productionItemsToExportForm.getItems().isEmpty()
				&& !differentNameAndSameUUIDAndIDForm.getItems().isEmpty()) {
			HarmonizeFormController.IS_NAMES_DIFFERENCES_HARMONIZED = false;
		}
	}
	...
	private void updateProductionToExportList(HarmonizationData productionItemsToExport) {
		...
		List<HarmonizationItem> itemsToRemove = getConvertedData(DTOUtils.fromForms(EXECUTED_FORMS_MANUALLY_CACHE))
				.getItems();
		productionItemsToExport.getItems().removeAll(itemsToRemove);
	}

	private void setSwappableDataClones(HttpSession session, List<Form> notSwappableForms, List<Form> swappableForms) {
	...
	public void removeAllChoosenToManualHarmonize(HttpSession session, List<Form> swappableForms) {
		Map<Form, Form> manualHarmonizeForms = (Map<Form, Form>) session.getAttribute("manualHarmonizeForms");
	...
	public void processAddNewFromMetadataServer(HarmonizationData onlyMetadataForms, Builder logBuilder) {
		...
		if (!list.isEmpty()) {
			...
			SUMMARY_EXECUTED_SCENARIOS.add("eptsharmonization.summary.encountertype.harmonize.onlyOnMDServer");
	...
	public void processDeleteFromProductionServer(List<FormDTO> list, Builder logBuilder) {
		if (!list.isEmpty()) {
			...
			SUMMARY_EXECUTED_SCENARIOS.add("eptsharmonization.summary.encountertype.harmonize.onlyOnPServer.unused");
	...
	public void processUpdateFormNames(HarmonizationData data, Builder logBuilder) {
		...
		if (!list.isEmpty()) {
			...
			SUMMARY_EXECUTED_SCENARIOS
					.add("eptsharmonization.summary.encountertype.harmonize.differentNamesAndSameUUIDAndID");
	...
	public void processFormsWithDiferrentIdsAndEqualUUID(HarmonizationData data, Builder logBuilder) {
		...
		if (!list.isEmpty()) {
			...
			SUMMARY_EXECUTED_SCENARIOS
					.add("eptsharmonization.summary.encountertype.harmonize.differentID.andEqualUUID");
