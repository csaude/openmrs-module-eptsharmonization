<c:if
	...
	<c:if test="${ not empty differentProgramsOrConceptsAndSameUUIDAndID.items}">

		<springform:form modelAttribute="differentProgramsOrConceptsAndSameUUIDAndID"
			method="post" action="processHarmonizationStep3.form">
			<b class="boxHeader"><spring:message
					code="eptsharmonization.programworkflow.harmonize.differentProgramsOrConceptsAndSameUUIDAndID" /></b>
			<fieldset>
				<table cellspacing="0" border="0" style="width: 100%">
					...
					<c:forEach var="item"
						items="${differentProgramsOrConceptsAndSameUUIDAndID.items}"
						varStatus="itemsRow">
						<tr class="confirm-each-row-harmonization">
							<td valign="top">${item.value[0].program}</td>
							<td valign="top">${item.value[0].concept}</td>
							<td valign="top">${item.value[1].program}</td>
							<td valign="top">${item.value[1].concept}</td>
							<td valign="top">${item.value[0].programWorkflow.id}</td>
