<c:if
	...
	<c:if
		...
		<div class="box">
			<table cellspacing="0" border="0" style="width: 100%">
				<tr>
					<th colspan="7" style="text-align: center; width: 45%;"><spring:message
							code="eptsharmonization.patientidentifiertype.from.metadataServer" /></th>
					<th colspan="7" style="text-align: center; width: 45%;"><spring:message
							code="eptsharmonization.patientidentifiertype.created.OnProductionServer" /></th>
				</tr>
				...
				<c:forEach var="item"
					items="${notSwappablePatientIdentifierTypesClone}"
					varStatus="itemStatus">
					<tr>
						<td valign="top" align="center">${item.id}</td>
						<td valign="top">${item.name}</td>
						<td valign="top">${item.description}</td>
						<td valign="top">${item.format}</td>
						<td valign="top">${item.checkDigit}</td>
						<td valign="top">${item.required}</td>
						<td valign="top">${item.uuid}</td>
						<c:choose>
							<c:when
								test="${not empty swappablePatientIdentifierTypesClone[itemStatus.index]}">
								<td>${swappablePatientIdentifierTypesClone[itemStatus.index].id}</td>
								<td>${swappablePatientIdentifierTypesClone[itemStatus.index].name}</td>
								<td>${swappablePatientIdentifierTypesClone[itemStatus.index].description}</td>
								<td>${swappablePatientIdentifierTypesClone[itemStatus.index].format}</td>
								<td>${swappablePatientIdentifierTypesClone[itemStatus.index].checkDigit}</td>
								<td>${swappablePatientIdentifierTypesClone[itemStatus.index].required}</td>
								<td>${swappablePatientIdentifierTypesClone[itemStatus.index].uuid}</td>
							</c:when>
							<c:otherwise>
								<td colspan="7"></td>
							</c:otherwise>
