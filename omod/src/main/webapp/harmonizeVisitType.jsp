...
<script>
    $j(document).ready(function(){
        $j('#submit-proceed').click(function(){
            ...
            if(!check){
                alert('<spring:message code="eptsharmonization.visittype.harmonize.ensureConfirmation"/>');
                return false;
			}
        });
    });
...
<form id="harmonize-visit-form" method="POST" class="box" action="mandatoryVisitTypeHarmonization.form">
<c:if test="${not empty missingInPDS}">
	<b class="boxHeader"><spring:message code="eptsharmonization.visittype.harmonize.missingInPDS" /></b>
		<table cellspacing="0" border="0" style="width: 100%">
			...
			<c:forEach var="item" items="${missingInPDS}">
				<tr>
					<td valign="top" align="center">${item.visitType.id}</td>
					<td valign="top">${item.visitType.name}</td>
					<td valign="top">${item.visitType.description}</td>
					<td valign="top">${item.visitType.uuid}</td>
				</tr>
			</c:forEach>
		</table>
	<br />
...
<c:if test="${not empty notInMDSNotUsed}">
	<b class="boxHeader"><spring:message code="eptsharmonization.visittype.harmonize.notInMDSNotUsed" /></b>
...
<c:if test="${not empty sameIdAndUuidDifferentNames}">
	<b class="boxHeader"><spring:message code="eptsharmonization.visittype.harmonize.sameIdAndUuidDifferentNames" /></b>
		<table class="box" cellspacing="0" border="0" style="width: 100%">
			<tr>
				<th><spring:message code="general.id" /></th>
				<th><spring:message code="general.uuid" /></th>
				...
				<th><spring:message code="eptsharmonization.visittype.harmonize.affectedVisits" /></th>
				<th colspan="2"><spring:message code="eptsharmonization.visittype.harmonize.actionForSimilarUuid" /></th>
			</tr>
			<c:forEach var="entry" items="${sameIdAndUuidDifferentNames}">
				<tr>
					...
					<td valign="top">${entry.key[0].visitType.name}</td>
					<td valign="top">${entry.key[0].visitType.description}</td>
					<td valign="top">${entry.key[1].visitType.name}</td>
					<td valign="top">${entry.key[1].visitType.description}</td>
...
<c:if test="${not empty sameUuidDifferentIdsAndNames}">
	<b class="boxHeader"><spring:message code="eptsharmonization.visittype.harmonize.sameUuidDifferentIds" /></b>
		<table class="box" cellspacing="0" border="0" style="width: 100%">
			<tr>
				<th><spring:message code="general.uuid" /></th>
			...
			<c:forEach var="entry" items="${sameUuidDifferentIdsAndNames}">
				<tr>
					...
					<td valign="top">${entry.key[0].visitType.name}</td>
					<td valign="top">${entry.key[0].visitType.description}</td>
					<td valign="top">${entry.key[1].visitType.id}</td>
					<td valign="top">${entry.key[1].visitType.name}</td>
					<td valign="top">${entry.key[1].visitType.description}</td>
					<td valign="top" align="center">${entry.value}</td>
					<td><input type="radio" value="true" name="${entry.key[0].uuid}"/><spring:message code="general.yes"/></td>
