	...
	<c:if test="${not empty differentIDsAndEqualUUID.items}">
		<springform:form modelAttribute="differentIDsAndEqualUUID"
			...
			<b class="boxHeader"><spring:message
					code="eptsharmonization.program.harmonize.differentID.andEqualUUID" /></b>
			<fieldset>
				<table cellspacing="0" border="0" style="width: 100%">
					...
					<c:forEach var="item" items="${differentIDsAndEqualUUID.items}"
						...
						<tr class="confirm-each-row-harmonization">
							<td valign="top" style="text-align: left; width: 5%;">${item.value[0].program.id}</td>
							<td valign="top" style="text-align: left; width: 15%;">${item.value[0].program.name}</td>
							<td valign="top" style="text-align: left; width: 15%;">${item.value[0].program.description}</td>
							<td valign="top" style="text-align: left; width: 5%;">${item.value[1].program.id}</td>
							<td valign="top" style="text-align: left; width: 15%;">${item.value[1].program.name}</td>
							<td valign="top" style="text-align: left; width: 15%;">${item.value[1].program.description}</td>
