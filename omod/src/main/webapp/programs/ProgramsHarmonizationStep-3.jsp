	...
	<c:if test="${ not empty differentNameAndSameUUIDAndID.items}">
		...
		<springform:form modelAttribute="differentNameAndSameUUIDAndID"
			...
			<b class="boxHeader"><spring:message
					code="eptsharmonization.program.harmonize.differentNamesAndSameUUIDAndID" /></b>
			<fieldset>
				<table cellspacing="0" border="0" style="width: 100%">
					...
					<c:forEach var="item"
						...
						<tr class="confirm-each-row-harmonization">
							<td valign="top">${item.value[0].program.name}</td>
							<td valign="top">${item.value[0].program.description}</td>
							<td valign="top">${item.value[1].program.name}</td>
							<td valign="top">${item.value[1].program.description}</td>
							<td valign="top">${item.value[0].program.id}</td>
