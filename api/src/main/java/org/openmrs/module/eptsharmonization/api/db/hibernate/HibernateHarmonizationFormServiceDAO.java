...
public class HibernateHarmonizationFormServiceDAO implements HarmonizationFormServiceDAO {
	...
	@Autowired
	public HibernateHarmonizationFormServiceDAO(SessionFactory sessionFactory, FormDAO formDAO,
			HarmonizationServiceDAO harmonizationServiceDAO, EncounterDAO encounterDAO) {
	...
	public List<Form> findPDSFormsNotExistsInMDServer() throws DAOException {
		final Query query = this.sessionFactory.getCurrentSession()
				.createSQLQuery("select form.* from form where NOT EXISTS (select * from _form "
	...
	public boolean isSwappable(Form form) throws DAOException {
		return (boolean) this.sessionFactory.getCurrentSession()
				.createSQLQuery(String.format("select swappable from form where form_id = %s ", form.getId()))
				.uniqueResult();
	}
	...
	public List<FormFilter> findFormFilterByForm(Form form) throws DAOException {
		...
		final Query query = this.sessionFactory.getCurrentSession().createSQLQuery(
				"select f.form_filter_id, f.form_id from formfilter_form_filter f where f.form_id=" + form.getFormId());
	...
	public List<FormentryXsn> findFormentryXsnByForm(Form form) throws DAOException {
		this.harmonizationServiceDAO.evictCache();
		final Query query = this.sessionFactory.getCurrentSession().createSQLQuery(
				"select f.formentry_xsn_id, f.form_id from formentry_xsn f where f.form_id =" + form.getFormId());
	...
	public HtmlForm findHtmlFormByForm(Form form) throws DAOException {
		...
		try {
			...
			while (rs.next()) {
				htmlForm = new HtmlForm(rs.getInt("id"), rs.getInt("form_id"), rs.getString("xml_data"),
						rs.getString("uuid"), rs.getBoolean("retired"), rs.getInt("creator"),
	...
	public List<FormResource> findFormResourcesByForm(Form form) throws DAOException {
		this.harmonizationServiceDAO.evictCache();
		final Query query = this.sessionFactory.getCurrentSession()
				.createSQLQuery("select fr.* from form_resource fr where fr.form_id = " + form.getFormId())
	...
	private Integer getNextFormId() {
		Integer maxId = (Integer) this.sessionFactory.getCurrentSession()
				.createSQLQuery("select max(form_id) from form ").uniqueResult();
		return ++maxId;
	}

	@SuppressWarnings("unchecked")
	private List<Form> findMDSForms() {
		final Query query = this.sessionFactory.getCurrentSession().createSQLQuery("select f.* from _form f")
				.addEntity(Form.class);
	...
	public List<Form> findAllSwappable() throws DAOException {
		return this.setRelatedMetadataFromTableForm(this.sessionFactory.getCurrentSession()
				.createSQLQuery(String.format("select form.* from form where swappable = true ")).addEntity(Form.class)
				.list());
	}
	...
	public List<Form> findAllNotSwappable() throws DAOException {
		return this.setRelatedMetadataFromTableForm(this.sessionFactory.getCurrentSession()
				.createSQLQuery(String.format("select form.* from form where swappable = false ")).addEntity(Form.class)
				.list());
	}
	...
	public List<HtmlForm> findHtmlFormPDSWithDifferentFormAndEqualUuidFromMDS() {
		...
		String sql = "SELECT id, form_id, xml_data, uuid, retired, creator, date_created FROM htmlformentry_html_form where EXISTS (select * from _htmlformentry_html_form          "
		...
		try {
			...
			rs = prepareStatement.executeQuery();
			while (rs.next()) {
				HtmlForm htmlForm = new HtmlForm(rs.getInt("id"), rs.getInt("form_id"), rs.getString("xml_data"),
						rs.getString("uuid"), rs.getBoolean("retired"), rs.getInt("creator"),
						new Date(rs.getDate("date_created").getTime()));
	...
	public Form updateForm(Form pdsForm, Form mdsForm, boolean swappable) throws DAOException {
		String updateSql = "update form set form_id = :formId, name = :name, encounter_type = :encounterType, swappable = :swappable, uuid = :uuid where form_id = :baseFormId";
		...
		final Criteria searchCriteria = this.sessionFactory.getCurrentSession().createCriteria(Form.class, "form");
	...
	public void saveNotSwappableForm(Form form) throws DAOException {
		...
		String insert = "insert into form (form_id, name, version, build, published, description, encounter_type, creator, date_created, retired, uuid, swappable) "
	...
	public void createHtmlForm(HtmlForm htmlForm) throws DAOException {
		...
		String insert = "insert into htmlformentry_html_form (form_id, xml_data, creator, date_created, retired, uuid) "
				+ " values (?, ?, ?, ?, ?, ?)";
	...
	public void updateHtmlForm(HtmlForm pdsHtmlForm, HtmlForm mdsHtmlForm) throws DAOException {
		...
		String update = "update htmlformentry_html_form set form_id =:newFormId where form_id = :formId";
	...
	public Form updateToNextAvailableId(Form form) throws DAOException {
		...
		this.sessionFactory.getCurrentSession()
				.createSQLQuery(String.format("update form set form_id =%s, swappable = true where form_id =%s ",
	...
	public void deleteForm(Form form) throws DAOException {
		this.sessionFactory.getCurrentSession()
				.createSQLQuery(String.format("delete from form where form_id =%s ", form.getFormId())).executeUpdate();
		this.sessionFactory.getCurrentSession().flush();
	}
	...
	public void deleteRelatedHtmlForm(Form form) throws DAOException {
		this.sessionFactory.getCurrentSession()
				.createSQLQuery(
						String.format("delete from htmlformentry_html_form where form_id =%s ", form.getFormId()))
				.executeUpdate();
		this.sessionFactory.getCurrentSession().flush();
	}
	...
	public void deleteRelatedFormFilter(Form form) throws DAOException {
		this.sessionFactory.getCurrentSession()
				.createSQLQuery(
						String.format("delete from formfilter_form_filter where form_id =%s ", form.getFormId()))
				.executeUpdate();
		this.sessionFactory.getCurrentSession().flush();
	}
	...
	public void deleteRelatedFormResource(Form form) throws DAOException {
		this.sessionFactory.getCurrentSession()
				.createSQLQuery(String.format("delete from form_resource where form_id =%s ", form.getFormId()))
				.executeUpdate();
		this.sessionFactory.getCurrentSession().flush();
	}
	...
	public void deleteRelatedFormentryXsn(Form form) throws DAOException {
		this.sessionFactory.getCurrentSession()
				.createSQLQuery(String.format("delete from formentry_xsn where form_id =%s ", form.getFormId()))
				.executeUpdate();
		this.sessionFactory.getCurrentSession().flush();
	}
	...
	public void deleteRelatedFormField(Form form) throws DAOException {
		this.sessionFactory.getCurrentSession()
				.createSQLQuery(String.format("delete from form_field where form_id =%s ", form.getFormId()))
				.executeUpdate();
		this.sessionFactory.getCurrentSession().flush();
	}
	...
	public void updateEncounter(Encounter encounter, Form form) throws DAOException {
		this.sessionFactory.getCurrentSession()
				.createSQLQuery(String.format("update encounter set form_id =%s where encounter_id =%s ",
	...
	public void updateFormField(FormField formField, Form form) throws DAOException {
		this.sessionFactory.getCurrentSession()
				.createSQLQuery(String.format("update form_field set form_id =%s where form_field_id =%s ",
	...
	public void updateFormResource(FormResource formResource, Form form) throws DAOException {
		this.sessionFactory.getCurrentSession()
				.createSQLQuery(String.format("update form_resource set form_id =%s where form_resource_id =%s ",
	...
	public void updateFormentryxsn(FormentryXsn formentryXsn, Form form) throws DAOException {
		this.sessionFactory.getCurrentSession()
				.createSQLQuery(String.format("update formentry_xsn set form_id =%s where formentry_xsn_id =%s ",
	...
	public void updateFormFilter(FormFilter formFilter, Form form) throws DAOException {
		this.sessionFactory.getCurrentSession()
				.createSQLQuery(String.format("update formfilter_form_filter set form_id =%s where form_filter_id =%s ",
	...
	public void updateHtmlForm(HtmlForm htmlForm, Form form) throws DAOException {
		this.sessionFactory.getCurrentSession()
				.createSQLQuery(String.format("update htmlformentry_html_form set form_id =%s where id =%s ",
	...
	private HtmlForm getMDSHtmlForm(Form form) {
		...
		String sql = "select id, form_id, xml_data, uuid, retired, creator, date_created FROM _htmlformentry_html_form where form_id = :formId";
	...
	private Integer findRelatedEncounterTypeFromTableMDSForm(Form form) throws DAOException {
		final Query query = this.sessionFactory.getCurrentSession().createSQLQuery(
				String.format("select _form.encounter_type from _form where _form.form_id = %s", form.getFormId()));
		return (Integer) query.uniqueResult();
	}

	private Integer findRelatedEncounterTypeFromTableForm(Form form) throws DAOException {
		final Query query = this.sessionFactory.getCurrentSession().createSQLQuery(
				String.format("select form.encounter_type from form where form.form_id = %s", form.getFormId()));
		return (Integer) query.uniqueResult();
	}
	...
	private List<HtmlForm> convertToHtmlFormObjects(List<Object[]> objects) {
		...
		for (Object[] object : objects) {
			...
			HtmlForm htmlForm = new HtmlForm((Integer) object[0], (Integer) object[1], (String) object[2],
					(String) object[3], (Boolean) object[4], (Integer) object[5], (Date) object[6]);
