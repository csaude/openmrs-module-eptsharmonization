...
public class HarmonizationProgramServiceImpl extends BaseOpenmrsService
		implements HarmonizationProgramService, HarmonizationService {
	...
	@Autowired
	public void setHarmonizationProgramServiceDAO(HarmonizationProgramServiceDAO harmonizationProgramServiceDAO) {
	...
	public List<ProgramDTO> findAllMetadataProgramsNotContainedInProductionServer() throws APIException {
	...
	@Transactional(readOnly = true)
	@Authorized({ "View Program" })
	public List<ProgramDTO> findAllProductionProgramsNotContainedInMetadataServer() throws APIException {
		this.harmonizationDAO.evictCache();
		List<Program> pdsPrograms = harmonizationProgramServiceDAO.findAllProductionServerPrograms();
		List<Program> mdsPrograms = harmonizationProgramServiceDAO.findAllMetadataServerPrograms();
	...
	@Transactional(readOnly = true)
	@Authorized({ "View Program" })
	public List<ProgramDTO> findAllMetadataProgramsPartialEqualsToProductionServer() throws APIException {
		this.harmonizationDAO.evictCache();
	...
	@Transactional(readOnly = true)
	@Authorized({ "View Program" })
	public List<ProgramDTO> findAllProductionProgramsPartialEqualsToMetadataServer() throws APIException {
		this.harmonizationDAO.evictCache();
		List<Program> allPDS = harmonizationProgramServiceDAO.findAllProductionServerPrograms();
		List<Program> allMDS = harmonizationProgramServiceDAO.findAllMetadataServerPrograms();
	...
	@Transactional(readOnly = true)
	@Authorized({ "View Program" })
	public Map<String, List<ProgramDTO>> findAllProgramsWithDifferentNameAndSameUUIDAndID() throws APIException {
		this.harmonizationDAO.evictCache();
	...
	@Transactional(readOnly = true)
	@Authorized({ "View Program" })
	public Map<String, List<ProgramDTO>> findAllProgramsWithDifferentIDAndSameUUID() throws APIException {
		this.harmonizationDAO.evictCache();
		Map<String, List<ProgramDTO>> result = new HashMap<>();
	...
	public int getNumberOfAffectedPatientPrograms(ProgramDTO programDTO) {
		this.harmonizationDAO.evictCache();
		return harmonizationProgramServiceDAO.findPatientProgramsByProgramId(programDTO.getProgram().getProgramId())
				.size();
	}
	...
	public int getNumberOfAffectedProgramWorkflow(ProgramDTO programDTO) {
		this.harmonizationDAO.evictCache();
		return harmonizationProgramServiceDAO.findProgramWorkflowsByProgramId(programDTO.getProgram().getProgramId())
				.size();
	}
	...
	public void saveProgramsWithDifferentNames(Map<String, List<ProgramDTO>> programs) throws APIException {
		this.harmonizationDAO.evictCache();
				...
				if (found != null) {
					...
					if (!this.harmonizationProgramServiceDAO.isSwappable(found)) {
						...
						throw new APIException(String.format(
								...
								program.getId(), program.getUuid(), program.getName(), found.getUuid(),
								found.getName()));
					}
					List<PatientProgram> relatedPatientPrograms = this.harmonizationProgramServiceDAO
							.findPatientProgramsByProgramId(found.getId());
					List<ProgramWorkflow> relatedProgramWorkflows = this.harmonizationProgramServiceDAO
							.findProgramWorkflowsByProgramId(found.getId());
	...
	@Override
	@Authorized({ "Manage Program" })
	public void saveProgramsWithDifferentIDAndEqualUUID(Map<String, List<ProgramDTO>> mapPrograms) throws APIException {
		this.harmonizationDAO.evictCache();
		try {
				...
				if (foundMDS != null) {
					if (!this.harmonizationProgramServiceDAO.isSwappable(foundMDS)) {
						throw new APIException(String.format(
								...
								pdSProgram.getId(), pdSProgram.getUuid(), pdSProgram.getName(), mdServerProgramId));
					}
					List<PatientProgram> relatedPatientPrograms = this.harmonizationProgramServiceDAO
							.findPatientProgramsByProgramId(foundMDS.getId());
					List<ProgramWorkflow> relatedProgramWorkflows = this.harmonizationProgramServiceDAO
							.findProgramWorkflowsByProgramId(foundMDS.getId());
					this.updateToNextAvailableID(foundMDS, relatedPatientPrograms, relatedProgramWorkflows);
				}
				List<PatientProgram> relatedPatientPrograms = this.harmonizationProgramServiceDAO
						.findPatientProgramsByProgramId(pdSProgram.getId());
				List<ProgramWorkflow> relatedProgramWorkflows = this.harmonizationProgramServiceDAO
						.findProgramWorkflowsByProgramId(pdSProgram.getId());
				this.updateToGivenId(pdSProgram, mdServerProgramId, false, relatedPatientPrograms,
						relatedProgramWorkflows);
			}
		} catch (Exception e) {
			throw new APIException(e.getMessage(), e);
		} finally {
	...
	@Override
	@Authorized({ "Manage Program" })
	public void saveManualMapping(Map<Program, Program> mapPrograms) throws UUIDDuplicationException, SQLException {
		this.harmonizationDAO.evictCache();
		try {
			...
			for (Entry<Program, Program> entry : mapPrograms.entrySet()) {
				...
				Program foundMDSProgramByUuid = this.harmonizationProgramServiceDAO
						.getProgramByUuid(mdsProgram.getUuid());

				if ((foundMDSProgramByUuid != null && !foundMDSProgramByUuid.getId().equals(mdsProgram.getId()))
						&& (!foundMDSProgramByUuid.getId().equals(pdSProgram.getId())
								&& !foundMDSProgramByUuid.getUuid().equals(pdSProgram.getUuid()))) {

					throw new UUIDDuplicationException(String.format(
							" Cannot Update the Program '%s' to '%s'. There is one entry with NAME='%s', ID='%s' an UUID='%s' ",
							pdSProgram.getName(), mdsProgram.getName(), foundMDSProgramByUuid.getName(),
							foundMDSProgramByUuid.getId(), foundMDSProgramByUuid.getUuid()));
				}
				...
				if (mdsProgram.getUuid().equals(pdSProgram.getUuid())
					...
					List<PatientProgram> relatedPatientPrograms = this.harmonizationProgramServiceDAO
							.findPatientProgramsByProgramId(foundPDS.getId());

					List<ProgramWorkflow> relatedProgramWorkflows = this.harmonizationProgramServiceDAO
							.findProgramWorkflowsByProgramId(foundPDS.getId());

					for (ProgramWorkflow programWorkflow : relatedProgramWorkflows) {
						this.harmonizationProgramServiceDAO.updateProgramWorkflow(programWorkflow,
								mdsProgram.getProgramId());
					}
					for (PatientProgram patientProgram : relatedPatientPrograms) {
						this.harmonizationProgramServiceDAO.updatePatientProgram(patientProgram,
								mdsProgram.getProgramId());
					}
					this.harmonizationProgramServiceDAO.deleteProgram(foundPDS);

					Program foundMDSProgramByID = this.harmonizationProgramServiceDAO
							.getProgramById(mdsProgram.getId());
	...
	private List<Program> removeElementsWithDifferentIDsAndSameUUIDs(List<Program> mdsPrograms,
			List<Program> pdsPrograms) {
	...
	private Map<String, List<Program>> findByWithDifferentNameAndSameUUIDAndID() {
		...
		for (Program mdsItem : allMDS) {
			for (Program pdsItem : allPDS) {
				if (mdsItem.getUuid().equals(pdsItem.getUuid()) && mdsItem.getId().equals(pdsItem.getId())
	...
	private void updateToGivenId(Program program, Integer programId, boolean swappable,
			List<PatientProgram> relatedPatientPrograms, List<ProgramWorkflow> relatedProgramWorkflows) {
	...
	private void updateToNextAvailableID(Program program, List<PatientProgram> relatedPatientPrograms,
		...
		for (ProgramWorkflow programWorkflow : relatedProgramWorkflow) {
			this.harmonizationProgramServiceDAO.updateProgramWorkflow(programWorkflow, updated.getProgramId());
		}
		for (PatientProgram patientProgram : relatedPatientPrograms) {
			this.harmonizationProgramServiceDAO.updatePatientProgram(patientProgram, updated.getProgramId());
		}
	}
	...
	public boolean isHarmonized() throws APIException {
		return findAllMetadataProgramsNotContainedInProductionServer().isEmpty()
				&& findAllMetadataProgramsPartialEqualsToProductionServer().isEmpty()
				&& findAllProductionProgramsNotContainedInMetadataServer().isEmpty()
				&& findAllProductionProgramsPartialEqualsToMetadataServer().isEmpty()
